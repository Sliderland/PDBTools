// Local baked-prior variant of statinvertprior.stan.
...
transformed data {
  ...
  es[1] = rep_vector(0.0, 2);
  es[2] = rep_vector(0.0, 2);
  fs[1] = rep_vector(sqrt(0.455), 2);
  fs[2] = rep_vector(sqrt(0.455), 2);
  gs[1] = rep_vector(1.365, 2);
  gs[2] = rep_vector(1.365, 2);
  hs[1] = rep_vector(0.071175, 2);
  hs[2] = rep_vector(0.071175, 2);
  scale_diag = 1.0;
  scale_offdiag = 0.0;
  df = m + 4;
  for (i in 1 : m) {
