...
pdbtools_rename_relpath <- function(root, path) {
  ...
  if (
    ...
    stop(
      "PosteriorDB paths must be relative and cannot contain '..'.",
      call. = FALSE
    )
  }
  file.path(root, path)
...
pdbtools_rename_infer_type <- function(root, old_name, type = NULL) {
  ...
  exists <- c(
    data = file.exists(file.path(
      root,
      "data",
      "info",
      paste0(old_name, ".info.json")
    )),
    model = file.exists(file.path(
      root,
      "models",
      "info",
      paste0(old_name, ".info.json")
    )),
    posterior = file.exists(file.path(
      root,
      "posteriors",
      paste0(old_name, ".json")
    ))
    ...
    for (implementation in seq_along(implementations)) {
      ...
      for (field in fields) {
        ...
        if (!identical(value, new_value)) {
          ...
          model_info$model_implementations[[implementation]][[
            field
          ]] <- new_value
        }
      }
...
PDBEntryBuilder <- R6::R6Class(
  ...
  public = list(
    ...
    compute_reference_summary_statistics = function(stan_fit) {
      ...
      draws <- posterior::subset_draws(
        self$get_fit_draws(stan_fit),
        variable = names(self$get_posterior_dims(self$get_fit_model_name(
          stan_fit
        )))
      )
    ...
    write_summary_statistics_from_stan_fit = function(
      ...
      paths <- lapply(names(summaries), function(type) {
        private$get_summary_statistic_paths(
          self$get_fit_model_name(stan_fit),
          type
        )
      })
  ...
  private = list(
    ...
    get_sampling_version_info = function(backend = self$stan_backend) {
      ...
      if (identical(backend, "rstan")) {
        ...
        versions$cmdstanr_version <- paste(
          "cmdstanr",
