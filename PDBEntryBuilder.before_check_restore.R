...
PDBEntryBuilder <- R6Class(
    ...
    public = list(
        ...
        check_draws = function(rp, write_mean_ac = FALSE) {
            ...
            if (is.null(rpi)) {
                stop(
                    "Posterior Reference Draws do not have required posterior reference draw information attached.",
                    call. = FALSE
                )
            }
            ...
            # Rebuild the acceptance checks from the current reference-draw
            # definition. In particular, do not retain the obsolete
            # `ess_within_bounds` result used by older posteriordb-r releases.
            checks_made <- list()
            ...
            checks_made$no_divergent_transitions <-
                !anyNA(num_divergent) &&
                all(is.finite(num_divergent)) &&
                sum(num_divergent) == 0
            if (!checks_made$no_divergent_transitions) {
                stop(
            ...
            checks_made$ndraws_is_10k <-
                length(ndraws) == 1L &&
                !is.na(ndraws) &&
                ndraws == 10000

            nchains <- rpi$diagnostics$nchains
            checks_made$nchains_is_gte_4 <-
                length(nchains) == 1L &&
                !is.na(nchains) &&
                nchains >= 4
            ...
            if (length(finite_rhat) != length(rhat)) {
                message(
                    paste0(
                        "Some variables had undefined Rhat. This may be ",
                        "expected for constant or structurally constrained ",
                        "quantities; their draw values are checked separately."
                    )
                )
            }
            checks_made$r_hat_below_1_01 <-
                length(finite_rhat) > 0L &&
                all(finite_rhat < 1.01)
            ...
            draw_variables <- posterior::variables(rp)
            if (!setequal(named_vars, draw_variables)) {
                stop(
                    paste(
                        "Diagnostic variable names do not match the variables",
                        "in the reference draws."
                    ),
                    call. = FALSE
                )
            }

            # Always recompute this inexpensive-to-store but potentially stale
            # diagnostic when explicitly requested. Otherwise reuse it only if
            # it has the required positional alignment.
            recompute_ac <- write_mean_ac ||
                is.null(rpi$diagnostics$mean_lag1_ac) ||
                length(rpi$diagnostics$mean_lag1_ac) != length(named_vars)

            if (recompute_ac) {
                ordered_draws <- posterior::subset_draws(
                    rp,
                    variable = named_vars
                )
                mean_lag1_ac <- self$compute_mean_lag1_ac(ordered_draws)
                mean_lag1_ac <- unname(mean_lag1_ac[named_vars])
                rpi$diagnostics$mean_lag1_ac <- mean_lag1_ac
            } else {
                mean_lag1_ac <- rpi$diagnostics$mean_lag1_ac
            }

            ac_values_to_check <- mean_lag1_ac[!is.na(mean_lag1_ac)]
            checks_made$abs_mean_lag1_ac_below_0_05 <-
                length(ac_values_to_check) > 0L &&
                all(is.finite(ac_values_to_check)) &&
                all(ac_values_to_check < 0.05)

            failed_checks <- names(checks_made)[
                !vapply(checks_made, isTRUE, logical(1))
            ]
            checks_passed <- length(failed_checks) == 0L
            if (!checks_passed) {
                stop(
                    paste0(
