# Add Heaps VAR posteriors in computationally staged groups, excluding the
# expensive 20-variable stationary/invertible VARMA workflow by default.
#
# Work classes:
# - easy: 3- and 10-variable VARs, excluding statinvert_varma and ansleykohn_var
# - hard: every remaining statinvert_varma or ansleykohn_var and every
#   20-variable model
#
...
# The med20 VARMA job is intentionally excluded because its reference
# sampling is substantially more expensive than the other Heaps models.
...
# Save every completed RStan fit as an RDS, including fits that also pass and
# are written to PosteriorDB. When both flags are TRUE, each fit is saved once
# in all_fit_dir; failed_fit_dir is not used.
save_all_fits <- FALSE
...
build_model_data <- function(model_name, y) {
  ...
  if (length(missing_fields) > 0L || length(unnecessary_fields) > 0L) {
    stop(
      "Invalid model-specific data fields for `",
      model_name,
      "`.",
      call. = FALSE
    )
...
for (dataset_key in names(dataset_definitions)) {
  ...
  for (model_name in names(model_definitions)) {
    ...
    is_hard <- model_name %in%
      c("statinvert_varma", "ansleykohn_var") ||
...
if (skip_med20_statinvert_varma) {
  excluded <- vapply(
    workflow_entries,
    function(spec) {
      identical(spec$dataset_key, "med20") &&
        identical(spec$model_name, "statinvert_varma")
    },
    logical(1)
  )
  if (any(excluded)) {
    excluded_names <- names(workflow_entries)[excluded]
    workflow_entries <- workflow_entries[!excluded]
    message(
...
confirm_hard_jobs <- function(number_of_jobs) {
  if (!is.null(hard_confirmation)) {
    if (
      !is.logical(hard_confirmation) ||
        length(hard_confirmation) != 1L ||
